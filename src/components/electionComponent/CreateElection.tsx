...
export const CreateElection: React.FC = () => {
    // --- STATE QUẢN LÝ CUỘC BẦU CỬ ---
    const [title, setTitle] = useState('');
    const [description, setDescription] = useState('');
    const [totalRounds, setTotalRounds] = useState<number>(2); // Mặc định khởi tạo 2 vòng

    // [BỔ SUNG]: State quản lý chuỗi ảnh tạm thời của cuộc bầu cử tổng thể
    const [electionImageBase64, setElectionImageBase64] = useState('');

    // Mảng lưu trữ dynamic cấu hình của từng vòng
    const [roundsConfig, setRoundsConfig] = useState<RoundTimeConfig[]>([
        { roundNumber: 1, startTime: '', endTime: '', maxAdvanceCount: 5 },
        { roundNumber: 2, startTime: '', endTime: '', maxAdvanceCount: 1 }
    ]);

    // --- STATE QUẢN LÝ ỨNG CỬ VIÊN ---
    const [dbCandidates, setDbCandidates] = useState<Candidate[]>([]);
    const [localCandidates, setLocalCandidates] = useState<Candidate[]>([]);
    const [selectedCandidateIds, setSelectedCandidateIds] = useState<number[]>([]);

    // --- STATE KHU VỰC THÊM TỪNG ỨNG VIÊN (GIỮ NGUYÊN) ---
    ...
    const fetchDbCandidates = async () => {
        try {
            const data = await electionApi.getAllCandidates();
    ...
    // Tự động sinh danh sách khung cấu hình khi đổi số vòng (Từ 1 đến 10)
    const handleTotalRoundsChange = (rounds: number) => {
        ...
        for (let i = 1; i <= rounds; i++) {
            ...
            newConfigs.push(
                existingConfig ? existingConfig : { roundNumber: i, startTime: '', endTime: '', maxAdvanceCount: 1 }
            );
        }
        setRoundsConfig(newConfigs);
    };

    // Cập nhật giá trị dynamic cho từng vòng
    ...
    // [BỔ SUNG]: Hàm đọc file ảnh cuộc bầu cử sang Base64
    ...
    // Đọc file ảnh ứng cử viên sang Base64 phục vụ thêm nhanh
    ...
    // GỬI TOÀN BỘ CẤU HÌNH ĐỂ TẠO CUỘC BẦU CỬ NHIỀU VÒNG DYNAMIC
    const handleSubmitElection = async (e: React.FormEvent) => {
        ...
        // Validate kiểm tra dữ liệu các vòng
        for (const round of roundsConfig) {
            ...
            if (round.roundNumber < totalRounds && round.maxAdvanceCount <= 0) {
                Swal.fire("Cảnh báo", `Số người thắng ở Vòng ${round.roundNumber} phải lớn hơn 0!`, "warning");
                return;
            }
        }
        ...
        try {
            const payload = {
                ...
                description,
                totalRounds,
                base64Image: electionImageBase64, // [BỔ SUNG]: Gửi chuỗi ảnh cuộc bầu cử tổng thể lên Backend
                roundsTimeSettings: roundsConfig,
                candidateIds: selectedCandidateIds,
                newCandidates: localCandidates
            };

            console.log(">>> [FE] Gửi Request tạo chuỗi vòng bầu cử kèm bộ lọc và banner:", payload);
            ...
            setTitle('');
            setDescription('');
            setElectionImageBase64(''); // Clear ảnh cuộc bầu cử
            handleTotalRoundsChange(2);
            ...
            <div className="info-section">
                ...
                <form onSubmit={handleSubmitElection} className="edit-form">
                    ...
                    <div style={{ marginBottom: '25px', paddingBottom: '15px', borderBottom: '1px solid #eaedf1' }}>
                        ...
                        {/* [BỔ SUNG]: KHUNG CHỌN VÀ HIỂN THỊ ANH CUỘC BẦU CỬ */}
                        <div className="form-group" style={{ marginTop: '15px' }}>
                    ...
                    {/* LƯỚI TỔNG HỢP HIỂN THỊ TẤT CẢ ỨNG VIÊN */}
