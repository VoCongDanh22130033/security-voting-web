...
const ElectionInvite: React.FC = () => {
  ...
  const handleVerify = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const msg: string = error.response?.data || error.message || "Mã CCCD không đúng.";
      const isLocked = msg.includes("bị khóa");
      Swal.fire(
        isLocked ? "Tài khoản bị khóa" : "Xác thực thất bại",
        msg,
        isLocked ? "warning" : "error"
      );
    } finally {
      setIsLoading(false);
    }
